...
func TestUser(t *testing.T) {
	// 1. 定义请求体
	requestBody := map[string]any{"email": "nihonge@qq.com", "password": "123456", "confirm_password": "123456"}
	jsonBody, _ := json.Marshal(requestBody)

	// 2. 创建 HTTP 请求对象
	req, err := http.NewRequest("POST", "http://localhost:8888/user/register", bytes.NewBuffer(jsonBody))
	if err != nil {
