...
func TestAuth(t *testing.T) {
	...
	// 2. 创建 HTTP 请求对象
	req, err := http.NewRequest("POST", "http://localhost:8888/auth/token", bytes.NewBuffer(jsonBody))
	if err != nil {
	...
	//verify方法验证，验证token是否正确————————————————————————————————————————————————————————————————————————————————————
	jsonBody, _ = json.Marshal(responseBody)
	req, err = http.NewRequest("POST", "http://localhost:8888/auth/verify", bytes.NewBuffer(jsonBody))
	if err != nil {
		t.Fatalf("创建请求失败: %v", err)
	}
	req.Header.Set("Content-Type", "application/json")
